...
func getLineData(_ labels: [String], json: JSON!) -> LineChartData {
  ...
  for set in dataSets! {
    ...
    if tmp["values"].exists() {
      ...
      if tmp["circleHoleRadius"].exists() {
        dataSet.circleHoleRadius = CGFloat(tmp["circleHoleRadius"].floatValue);
      }
      ...
      if (tmp["fillColorTop"].exists() && tmp["fillColorBottom"].exists()){
        let startColor = UIColor(tmp["fillColorBottom"].stringValue);
        let endColor = UIColor(tmp["fillColorTop"].stringValue);
